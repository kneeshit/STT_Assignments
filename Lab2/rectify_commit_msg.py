...
def summarize_diff(diff_path, max_lines=5):
	"""Summarize the diff file for context (first few lines, or empty if not found)."""
...
def summarize_code(code_path, max_lines=5):
	"""Summarize the code file for context (first few lines, or empty if not found)."""
...
def rectifier(commit_msg, fix_type, diff_path, file_name, before_path, after_path):
	...
	diff_summary = summarize_diff(diff_path)
	before_summary = summarize_code(before_path)
	after_summary = summarize_code(after_path)
...
csv_path = "commit_predictions.csv"
